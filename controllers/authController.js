...
export const login = async (req, res) => {
    ...
    try {
        ...
        userPermissions.forEach(perm => {
             if (perm.resource === 'courses' || perm.resource === 'cursos' || perm.code.includes('COURSE')) {
                 if (perm.action === 'MANAGE' || perm.action === 'CREATE' || perm.action === 'DELETE' || perm.code.includes('MANAGE') || perm.code.includes('UPLOAD')) {
                     nivelCursos = Math.max(nivelCursos, 3); // ESCRITURA
                 } else if (perm.action === 'UPDATE') {
                     nivelCursos = Math.max(nivelCursos, 2); // ACTUALIZACION
                 } else if (perm.action === 'VIEW' || perm.code.includes('VIEW')) {
        ...
        const [userRoles] = await sequelize.query(`SELECT r.code FROM user_roles ur JOIN roles r ON ur.role_id = r.id WHERE ur.user_id = :userId`, {
            replacements: { userId: usuario.id }
        });
...
export const registrar = async (req, res) => {
    const { username, email, password, firstName, lastName, photoUrl } = req.body;
    ...
    try {
        ...
        // Buscar rol STUDENT
        const [roles] = await sequelize.query(`SELECT id FROM roles WHERE code = 'STUDENT' LIMIT 1`);
        if (roles.length > 0) {
        ...
        res.status(500).json({ error: error.message || "No se pudo registrar" });
    }
};

export const logout = async (req, res) => {
    const token = req.cookies.authToken || req.headers.authorization?.split(" ")[1] || req.body.token;
    if (token) {
